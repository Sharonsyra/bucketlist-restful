...
def create_app(config_name):
    ...
    def authorization(headers):
        if 'Authorization' not in headers.keys():
            return (make_response(json.dumps({"message": "No token"})), 401)

    @app.route('/api/v1.0/bucketlists/', methods=['POST', 'GET'])
    def bucketlists():
        head = authorization(request.headers)
        if head:
            return head
            ...
            if request.method == "POST":
                ...
                if not search:
                    ...
                    if paginated_buckets.has_next:
                        nextpage = 'http://127.0.0.1:5000/api/v1.0/bucketlists/?page=' + \
                            str(page + 1) + '&limit=' + str(limit)
                    else:
                        nextpage = ""
                    if paginated_buckets.has_prev:
                        previouspage = 'http://127.0.0.1:5000/api/v1.0/bucketlists/?page=' + \
                            str(page - 1) + '&limit=' + str(limit)
                    else:
                        previouspage = ""
                    results = []
            ...
            if bucketlist:
                if request.method == "POST":
                    item = Item.query.filter_by(
                        name=request.data['name'], bucketlist_id=id).first()
                    if not item:
                        name = str(request.data.get('name', ''))
                        if name:
