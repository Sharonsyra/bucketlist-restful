...
class AuthTestCase(unittest.TestCase):
    ...
    #     response = self.client().post("/api/v1.0/auth/register", data={'email':'sharon@gmail.com', 'password':'sha'})
    #     result = json.loads(response.data.decode())
    #     self.assertEqual(
    #         result['message'], "The length of the password should be at least six characters")
    #     self.assertEqual(response.status_code, 400)

    # def test_register_email_format(self):
    #     """Test that register uses right username format"""
    #     response = self.client().post("/api/v1.0/auth/register", data={'email':'sharonsyra', 'password':'sharon'})
    #     result = json.loads(response.data.decode())
    #     self.assertEqual(
    ...
    def test_registration(self):
        ...
        response = self.client().post('/api/v1.0/auth/register', data=self.user_data)
        result = json.loads(response.data.decode())
        self.assertEqual(
    ...
    def test_register_duplicates(self):
        ...
        response = self.client().post('/api/v1.0/auth/register', data=self.user_data)
        self.assertEqual(response.status_code, 201)
        second_response = self.client().post('/api/v1.0/auth/register', data=self.user_data)
    ...
    #     response = self.client().post('/api/v1.0/auth/login', data={'email':'', 'password':''})
    #     result = json.loads(response.data.decode())
    #     self.assertEqual(response.status_code, 400)
    #     self.assertEqual(
    #         result['message'], "These fields cannot be blank!")

    def test_user_login(self):
        """Test that registered users can login."""
        response = self.client().post('/api/v1.0/auth/register', data=self.user_data)
        self.assertEqual(response.status_code, 201)
        login_response = self.client().post('/api/v1.0/auth/login', data=self.user_data)
    ...
    def test_non_registered_user_login(self):
        ...
        response = self.client().post('/api/v1.0/auth/login', data=unauthorized_personel)
        result = json.loads(response.data.decode())
